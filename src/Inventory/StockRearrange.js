                        ...
                        <div className="mt-2 " style={{ overflowX: 'auto' }}>
                            ...
                            <Table responsive>
                                <thead style={{ background: '#ebeef0' }}>
                                    <tr>
                                        {
                                            Object.keys(stockrearrange[0]).map((d, i) => {
                                                return (
                                                    <td className="text-capitalize">{d === 'lane_assigned' || d === 'total_products' ? <div className='text-end'>{d.replace(/_/g, ' ')}</div> : d.replace(/_/g, ' ')}</td>
                                                );
                                            })
                                        }
                                        <th></th>
                                                ...
                                                <tr>
                                                    {
                                                        Object.keys(stockrearrange[0]).map((d, i) => {
                                                            ...
                                                            return (
                                                                <td>{d === "lane_assigned" || d === "total_products" ? <div className='text-end'>{data[d]}</div>
                                                                    : d === "rack_assigned" ? <span style={{ color: 'rgb(85 86 166)' }}>{data[d]}</span>
                                                                        : data[d] === "Completed" ?
